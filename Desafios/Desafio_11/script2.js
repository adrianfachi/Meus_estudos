const dataPartida = localStorage.getItem("dataPartida");
			...
			const dataEscolhidaPartida = `${dataPartida.slice(
				-2
			)}/${dataPartida.slice(5, 7)}/${dataPartida.slice(0, 4)}`;
			const dataEscolhidaRetorno = `${dataRetorno.slice(
				-2
			)}/${dataRetorno.slice(5, 7)}/${dataRetorno.slice(0, 4)}`;
			...
			selectCities.addEventListener("change", () => {
				localStorage.setItem(
					"LocalDestino",
					`${selectCities.value}, ${selectStates.value}`
				);
				calcRoute
			});
